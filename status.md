...
- **Hybrid MCTS + alpha-beta** — **shipped and on by default** (`tools/common/mcab.hpp`), +46.9 ±23.5 Elo over pure alpha-beta at 200ms/move. See the design note below. What remains:
  ...
  - The WASM/browser build (`gui_web/engine_wasm.cpp`) still runs pure alpha-beta — it includes `src/` only, and `mcab.hpp` deliberately lives in `tools/`. Wiring it up needs the time-control question above answered first, since the GUI often searches for well under 200ms.
...
### Hybrid MCTS + alpha-beta (`tools/common/mcab.hpp`)
