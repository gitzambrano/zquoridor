...
3. **Done in code (2026-08-15): AvgBlend is now production.** `BackupMode::MinimaxHard` recomputes each parent node's max backed child value before propagating to the ancestor, while `BackupMode::AvgBlend` implements the standard MCTS mean (`W += v`, `Q = W/N`). AvgBlend is now the production default after a strong arena result; arena and self-play still expose `--mcab-backup minimax|avg` so the hard mode remains available for comparison.
...
5. **Done in code (2026-08-15): progressive widening.** When enabled, MCAB cheaply enumerates candidates, ranks them by the NNUE policy head, and exposes only a prefix of `k = initial + coefficient · N^exponent` already-legal moves at each node. Path-preserving wall legality is checked only as candidates enter the active prefix; the prefix widens as visits grow. Production remains off by default (`initial=16`, `coefficient=2.0`, `exponent=0.5`) until an Elo gate. The first useful measurement is progressive widening on vs. off with `leafDepth=0` and `AvgBlend`.
...
- **2026-08-15**: Implemented the MCAB backup-mode experiment. `MinimaxHard` recomputes a node's max backed child value during backup; `AvgBlend` stores the conventional visit sum and uses `Q=W/N`. Added backup-mode parsing/configuration to `src/mcab.hpp`, `tools/arena/arena.cpp`, `tools/arena/run_arena.py`, and `tools/selfplay/selfplay_main.cpp`, with minimax-hard unchanged as the production default. Added `testBackupModes` to `tests/test_mcab_core.cpp`.
- **2026-08-15**: Corrected the MinimaxHard propagation to recompute the backed value of each parent node before inverting it toward the ancestor; the first implementation took a max directly over unrelated leaf samples on one edge and was too optimistic. A post-correction smoke match at 20ms/move (20 games, local minimax vs local AvgBlend) was still 0–18 with 2 draws, so AvgBlend remains a serious candidate, but this is not yet a production-strength result.
- **2026-08-15**: Promoted `BackupMode::AvgBlend` to the production default in `src/mcab.hpp`; explicit `--mcab-backup minimax` remains available for A/B testing.
- **2026-08-15**: Implemented optional MCAB progressive widening. The aggressive path enumerates pawn moves and cheap non-overlapping wall slots, ranks candidates with the NNUE policy head, and defers the path-preserving wall-legality check until a candidate enters the active prefix. `McabParams` exposes `progressiveWidening`, `wideningInitialMoves=16`, `wideningCoefficient=2.0`, and `wideningExponent=0.5`; arena and self-play config/CLI surfaces expose the feature, which remains off by default. Added `testProgressiveWidening`; the full test build and focused MCAB tests pass.
- **2026-08-15**: Corrected two progressive-widening integration issues found during validation. Active priors are now renormalized from the original candidate softmax rather than repeatedly renormalizing already-scaled values, and the Python arena config no longer forces Engine 2 into the experimental widening mode on ordinary runs. Both engines therefore use the production default unless the experiment is explicitly enabled.
- **2026-08-15**: Strength test of aggressive progressive widening at 150ms/move, 1000 games, 14 threads: widening 451-471-78 against the same engine with widening off, `-6.9 ±20.7 Elo` (inconclusive). Throughput was 10,345 vs. 12,796 nodes/s, about 19% lower with widening. Keep the feature experimental and off by default; no strength gain was demonstrated.
