...
double antitheticMatchAvg(const std::vector<TunableParam>& paramsPlus,
                           const std::vector<TunableParam>& paramsMinus,
                           std::mt19937& rng, int minDepth, int repeats, int numThreads) {
    repeats = std::max(1, repeats);
    std::vector<unsigned> seeds(repeats);
    for (int i = 0; i < repeats; i++) seeds[i] = rng();  // sementes tiradas SEQUENCIALMENTE do rng principal -> reprodutível independente do numero de threads
    ...
    auto worker = [&]() {
        ...
        while ((idx = nextIdx.fetch_add(1)) < repeats) {
            ...
            scores[(size_t)idx] = antitheticMatch(paramsPlus, paramsMinus, localRng, minDepth);
        }
    };
    if (nt <= 1) {
        worker();
...
void runSweepMinDepth(const std::vector<int>& candidates, int gamesPerPair,
    ...
    for (int i = 0; i < n; i++) {
        for (int j = i + 1; j < n; j++) {
            ...
            for (int g = 0; g < gamesPerPair; g++) seeds[(size_t)g] = rng();
            ...
            if (nt <= 1) {
                ...
                std::vector<std::thread> pool;
                pool.reserve((size_t)nt);
                for (int t = 0; t < nt; t++) pool.emplace_back(worker, &nextIdx);
                for (auto& th : pool) th.join();
            }

            double pairScore = 0.0;
            for (double s : gameScores) pairScore += s;
...
void runSpsa(std::vector<TunableParam> params, int totalIterations, unsigned seed,
             double timeBudgetSec, std::string ckptPath, std::string historyPath,
             std::string resultPath, int minDepth, int gamesPerIter, int numThreads,
    ...
    if (nActive == 0) {
        printf("%snenhum parametro ativo (--tune-contempt/--tune-policy-scale/--tune-cat-scale) -- nada a fazer.\n", logPrefix.c_str());
        return;
    }
    ...
    if (ckptIn) {
        ...
        if (nSaved == nActive) {
            ...
            printf("%scheckpoint incompativel com o conjunto de parametros ativo (esperava %d, achou %d) -- ignorando e comecando do zero.\n", logPrefix.c_str(), nActive, nSaved);
            kStart = 0; avgCount = 0;
        }
        fclose(ckptIn);
    }
    ...
    if (kStart == 0) {
        printf("%sSPSA -- %d iteracoes totais, %d partida(s) antitetica(s)/iteracao (%d thread(s)), profundidade=%d, %dms/lance, %d plies aleatorios de abertura, NNUE=%s policyOrdering=%s(minDepth=%d)\n",
               logPrefix.c_str(), totalIterations, std::max(1, gamesPerIter), std::max(1, numThreads), g_searchDepth, g_timeBudgetMs, g_openingRandomPlies,
               g_useNNUE ? "on" : "off", g_policyOrdering ? "on" : "off", minDepth);
    ...
    for (; k < totalIterations; k++) {
        ...
        double score = antitheticMatchAvg(paramsPlus, paramsMinus, rng, minDepth, gamesPerIter, numThreads);
        ...
        printf("%siter %3d/%d score=%+.2f elapsed=%.0fs theta: ", logPrefix.c_str(), k + 1, totalIterations, score, elapsed);
        ...
        FILE* hf = fopen(historyPath.c_str(), (freshHistory && k == kStart) ? "w" : "a");
        if (hf) {
            if (freshHistory && k == kStart) {
                fprintf(hf, "iter,elapsed_s,score");
                for (int i = 0; i < nActive; i++) fprintf(hf, ",%s", params[activeIdx[i]].name);
                fprintf(hf, "\n");
            }
            fprintf(hf, "%d,%.2f,%.4f", k, elapsed, score);
            for (int i = 0; i < nActive; i++) fprintf(hf, ",%.6f", theta[i]);
            fprintf(hf, "\n");
            fclose(hf);
        } else {
            fprintf(stderr, "%saviso: nao consegui abrir '%s' para gravar historico\n", logPrefix.c_str(), historyPath.c_str());
        }
    }
    ...
    if (k >= totalIterations) {
        printf("\n%s=== CONCLUIDO -- resultado (media de Polyak da 2a metade das iteracoes) ===\n", logPrefix.c_str());
        FILE* f = fopen(resultPath.c_str(), "w");
        for (int i = 0; i < nActive; i++) {
            printf("%s%s: %.4f (era %.4f)\n", logPrefix.c_str(), params[activeIdx[i]].name, thetaAvg[i], theta0[i]);
            if (f) fprintf(f, "%s %.6f\n", params[activeIdx[i]].name, thetaAvg[i]);
        }
        if (f) { fprintf(f, "# minDepth %d\n", minDepth); fclose(f); }
    } else {
        printf("\n%s=== lote parcial salvo em checkpoint (%d/%d iteracoes feitas) -- rode de novo pra continuar ===\n", logPrefix.c_str(), k, totalIterations);
    }
}
...
void runHybrid(const std::vector<int>& candidates, const std::vector<TunableParam>& baseParams,
               int totalIterations, unsigned seed, double timeBudgetSec,
               const std::string& ckptPath, const std::string& historyPath, const std::string& resultPath,
               int gamesPerIter, int numThreads) {
    printf("=== HIBRIDO: 1 thread de SO por policyOrderingMinDepth -- candidatos: ");
    for (int d : candidates) printf("%d ", d);
    printf("===\n");
    printf("cada thread roda um SPSA completo (%d iteracoes, %d partida(s) antitetica(s)/iteracao) com\n"
           "policyOrderingMinDepth FIXO no seu valor. --threads=%d aplica DENTRO de cada SPSA\n"
           "(paraleliza as %d partidas de uma iteracao); os %zu candidatos SEMPRE rodam\n"
           "concorrentemente entre si (1 thread do SO cada) -- cuidado com oversubscription\n"
           "se --threads * numero_de_candidatos > nucleos disponiveis.\n\n",
           totalIterations, std::max(1, gamesPerIter), std::max(1, numThreads), std::max(1, gamesPerIter), candidates.size());
    ...
    for (size_t i = 0; i < candidates.size(); i++) {
        ...
        pool.emplace_back(runSpsa, baseParams, totalIterations, candSeed, timeBudgetSec,
                           ckpt, hist, res, d, gamesPerIter, numThreads, prefix);
    }
    for (auto& th : pool) th.join();

    printf("\n=== HIBRIDO concluido -- resultados/historicos por depth: %s / %s (sufixo _depth{D}) ===\n",
           withSuffix(resultPath, "_depth{D}").c_str(), withSuffix(historyPath, "_depth{D}").c_str());
    printf("Compare os thetaAvg finais e a curva de 'score' da 2a metade de cada historico\n"
           "(python3 teste/plot_spsa.py --history %s --glob) pra escolher o minDepth+parametros final;\n"
           "opcionalmente confirme o vencedor com um --mode sweep-mindepth ou teste/arena.cpp de verdade.\n",
           historyPath.c_str());
}
...
void printUsage(const char* prog) {
    ...
    printf("  --iterations N                   (default: 100, modos spsa/hybrid)\n");
    printf("  --games-per-iter N               partidas antiteticas por iteracao SPSA,\n");
    printf("                                    media reduz o ruido do gradiente (default: 4)\n");
    printf("  --threads N                      paralelismo dentro de 1 iteracao/par\n");
    printf("                                    (spsa/sweep-mindepth) ou dentro de 1 candidato\n");
    ...
    printf("  --history PATH                   historico CSV p/ plot_spsa.py (default: spsa_history.csv)\n");
    printf("  --result PATH                    resultado final (default: spsa_result.txt)\n");
    ...
    printf("  --policy-order-min-depth N       (default: 3; modos spsa/sweep-mindepth-fixo)\n");
    ...
    printf("  --mindepth-candidates D1,D2,...  (default: 1,2,3,4,5 -- modos sweep-mindepth/hybrid)\n");
...
int main(int argc, char** argv) {
    ...
    int totalIterations = 100;          // 2026-08: era 40 -- "mais jogos, mais estatistica"
    int gamesPerIter = 4;               // 2026-08 (NOVO): 4 partidas antiteticas/iteracao (8 jogos)
    int numThreads = 1;
    ...
    for (int i = 1; i < argc; i++) {
        ...
        else if (a == "--games-per-iter") gamesPerIter = std::atoi(next().c_str());
        else if (a == "--threads") numThreads = std::atoi(next().c_str());
        ...
        else if (a == "--history") historyPath = next();
        else if (a == "--result") resultPath = next();
        ...
        else if (a == "--policy-order-min-depth") policyOrderMinDepth = std::atoi(next().c_str());
    ...
    if (gamesPerIter < 1) gamesPerIter = 1;
    ...
    if (mode == "spsa") {
        runSpsa(params, totalIterations, seed, timeBudgetSec, ckptPath, historyPath, resultPath,
                policyOrderMinDepth, gamesPerIter, numThreads, "");
    } else if (mode == "sweep-mindepth") {
        runSweepMinDepth(mindepthCandidates, mindepthGames, params, seed, numThreads);
    } else if (mode == "hybrid") {
        ...
        fprintf(stderr, "--mode desconhecido: '%s' (use spsa, sweep-mindepth ou hybrid)\n", mode.c_str());
        return 1;
    }
    return 0;
}
