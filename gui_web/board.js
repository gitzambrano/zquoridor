...
(function (global) {
    ...
    var THEMES = {
        wood: {
            cellLight: '#e8cfa0', cellDark: '#c9a678', boardBg: '#4a331f',
            frame: '#3a2717', gridLine: 'rgba(74,51,31,0.28)', grain: 0.55,
            wall: '#8a5a2b', wallEdge: '#5c3a17', wallShadow: 'rgba(28,16,6,0.55)',
            coord: '#f0dcb4', dot: 'rgba(40,26,12,0.42)', lastMove: 'rgba(214,178,52,0.42)',
            pathP0: '#f4e7c2', pathP1: '#e0725f',
            p0: '#f6ecd2', p0Edge: '#9c8144', p1: '#b8323f', p1Edge: '#5e1119',
            goalGlow: 'rgba(230,201,110,0.55)', arrow: 'rgba(240,190,70,0.75)'
        },
        classic: {
    ...
    QBoard.prototype._drawLastWallTint = function (ctx) {
        ...
        if (!lm || !lm.isWall) return;
        var g = this._g, t = this.theme;
        var q = this._slotRect(lm.a | 0, lm.b | 0, lm.c | 0);
        ctx.save();
        roundRectPath(ctx, q.x - 1.5, q.y - 1.5, q.w + 3, q.h + 3, Math.max(3, g.r));
        ctx.strokeStyle = rgba(t.lastMove, 0.85);
        ctx.lineWidth = 2;
        ctx.stroke();
        ctx.restore();
    };
    ...
    QBoard.prototype._paintWall = function (ctx, o, r, c, owner, alpha, glow, lift) {
        ...
        var elev = Math.max(1.5, g.gap * 0.45) + lift;
