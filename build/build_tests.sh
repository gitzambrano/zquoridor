...
echo "[10/14] test_search_leaf_smoke  (Fase 0: searchLeaf/resetOrderingState)"
g++ "${FLAGS[@]}" -I"$SRC" -o "$BIN/test_search_leaf_smoke" "$TESTS/test_search_leaf_smoke.cpp"

echo "[11/14] test_mcab_core  (scoreToQ, budget do pool, sinal do backup, modo equivalência)"
g++ "${FLAGS[@]}" -I"$SRC" -o "$BIN/test_mcab_core" "$TESTS/test_mcab_core.cpp"

echo "[12/14] test_mcab_dispatch  (SFINAE: refs antigas sem searchLeaf caem no AB puro)"
g++ "${FLAGS[@]}" -I"$SRC" -o "$BIN/test_mcab_dispatch" "$TESTS/test_mcab_dispatch.cpp"

echo "[13/14] test_mcab_phase9  (reuso de árvore, ruído Dirichlet, leaf depth adaptativa, teto de tempo)"
