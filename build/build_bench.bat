...
echo [8/10] bench_wall_ext.exe  ^<-  benchmarks\bench_wall_ext.cpp
g++ %FLAGS% -I"%SRC%" -o "%BIN%\bench_wall_ext.exe" "%BENCHMARK%\bench_wall_ext.cpp"
if errorlevel 1 goto :erro

REM [9] inv/ab-policy: compute map (policy/MCTS vs AB time attribution).
echo [9/10] map_compute.exe  ^<-  benchmarks\map_compute.cpp
g++ %FLAGS% -I"%SRC%" -o "%BIN%\map_compute.exe" "%BENCHMARK%\map_compute.cpp"
if errorlevel 1 goto :erro

REM [10] inv/ab-policy: nodes-to-fixed-depth / time-to-depth por variante.
echo [10/10] bench_policy_ab.exe  ^<-  benchmarks\bench_policy_ab.cpp
