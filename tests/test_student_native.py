...
def compile_probe(tmp_path, architecture):
    ...
    race = architecture in ("race", "multipath", "multipath_phase", "multipath_phase_contact")
...
def test_multipath_phase_contact_native_layout_matches_python(tmp_path):
    torch.manual_seed(31)
    baseline = Student("multipath_phase", 512, qat=True)
    candidate = Student("multipath_phase_contact", 512, qat=True)
    candidate.warm_start(baseline)
    with torch.no_grad():
        candidate.fc1.weight[:, 504:].normal_(0, 0.01)
    export(candidate, tmp_path / "multipath_phase_contact.bin")

    binary = compile_probe(tmp_path, "multipath_phase_contact")
    rows = probe(binary, tmp_path / "multipath_phase_contact_int8.bin")
    data = {key: np.asarray([r[key] for r in rows], dtype=np.uint64 if key.startswith("walls_") else np.int64)
            for key in ("own_pawn", "opp_pawn", "walls_h", "walls_v", "own_dist", "opp_dist",
                        "walls_left_own", "walls_left_opp")}
    with torch.no_grad():
        value, policy = candidate(torch.from_numpy(
            encode_features(data, np.arange(len(rows)), "multipath_phase_contact")))
    np.testing.assert_allclose(value.numpy(), [r["value"] for r in rows], atol=0.002, rtol=0)
    expected = np.asarray([r["policy"] for r in rows])
    valid = np.ones_like(expected, dtype=bool)
    valid[data["walls_left_own"] == 0, 81:] = False
