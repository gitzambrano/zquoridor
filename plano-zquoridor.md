...
| **Tier 3** | **Search Genérico em Larga Escala** | **1.000.000** | Estados representativos de partidas completas e auto-jogo auditado. Submetidos a busca real bilateral: Claustrophobia MCTS (64 simulações em CUDA) + ZQuoridor MCAB. Dá consistência tática em posições neutras. | **$\times 3,5$** | **100.000 com busca MCAB concluídos** (`generic-search-100k-zq/dataset.npz`). Part 1 (50k) **100% Concluído na GPU** (`part1_claustro.npz`); Part 2 (50k) em execução ativa na GPU CUDA (`task-3272`, 8 workers a 99%); 900.000 posições adicionais amostradas. |
| **Tier 3.5** | **Generic Critical Search (Nova Camada)** | **100.000** | Posições extraídas do acervo de auto-jogo genérico aplicando quatro filtros refinados de criticidade: alta entropia de política (Shannon entropy / top-2 ratio $\ge 0,5$), incerteza de valor e corrida ($|V| \approx 0$, $|d_{\text{own}} - d_{\text{opp}}| \le 2$), mudanças críticas de muro ($\le 3$ muros ou labirinto denso com alta pressão CAT) e swings de virada. **Supervisão com busca real em árvore ("search search mesmo")**: **80% Claustrophobia MCTS** (`zq_search_bridge.exe` 64 sims em CUDA) + **20% ZQuoridor MCAB** (512 nós em CPU), ponderados por divergência Jensen-Shannon. | **$\times 4,5$** | **100.000 posições mineradas e isoladas** (`tier3-5-generic-critical-100k/positions.jsonl`, 85k treino / 15k validação). Fila de busca engatilhada para 80/20. |
...
| **Tier 5** | **Branching Rollouts Dinâmicos** | **10.000 jogos + Deep Search** | Partidas completas geradas a partir de estados de crise com perturbação top-4 nos plies 1–4 e jogo determinístico MCAB subsequente. Alvos temporais descontados: $V = \text{sign} \cdot 0,98^{\text{plies\_restantes}}$. **Novo enriquecimento aprovado**: submissão de pontos de ramificação/crise do Tier 5 a busca profunda com Claustrophobia MCTS (~500ms / ~512 sims) na GPU logo após o término do Tier 3. | **$\times 6,0$** | **2.400 partidas em execução nos plies finais** (`task-1589`, PID 6764), **7.600 partidas configuradas** para 14 threads. Deep search na GPU engatilhado para a sequência. |
...
2. **Receita de Treinamento**:
   ...
   - Recursos de Hardware e Alocação Balanceada Homologada:
     ...
     - **Divisão Concorrente Atual**: 6 threads alocadas para rollouts (`generate_rollouts.exe` PID 6764) + 8 threads/workers alocadas para Claustrophobia MCTS (`zq_search_bridge.exe` via GPU CUDA) = **14 threads exatas**, 100% em conformidade com o teto.
     - **GPU (VRAM)**: NVIDIA GeForce RTX 4050 com **6 GB de VRAM**. Correção de driver realizada em `zq_inference_worker.py` (resolvendo aceitação de `cuda`/`gpu`), permitindo offload real para os núcleos Tensor da GPU.
       - Inferência MCTS ativa (8 workers): consumo medido de **1.221 MiB de VRAM**, mantendo 4,7 GB livres e zero risco de OOM ou throttling térmico.
