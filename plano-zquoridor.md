...
| **Tier 3.5** | **Generic Critical Search (Nova Camada)** | **100.000** | Posições extraídas do acervo de auto-jogo genérico aplicando quatro filtros refinados de criticidade: alta entropia de política (Shannon entropy / top-2 ratio $\ge 0,5$), incerteza de valor e corrida ($|V| \approx 0$, $|d_{\text{own}} - d_{\text{opp}}| \le 2$), mudanças críticas de muro ($\le 3$ muros ou labirinto denso com alta pressão CAT) e swings de virada. Supervisão bilateral completa (Claustrophobia MCTS + ZQ MCAB). | **$\times 4,5$** | **100.000 posições mineradas e isoladas** (`tier3-5-generic-critical-100k/positions.jsonl`, 85k treino / 15k validação). Fila de busca engatilhada. |
