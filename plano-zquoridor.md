...
| **Tier 3.5** | **Generic Critical Search (Nova Camada)** | **100.000** | Posições extraídas do acervo de auto-jogo genérico aplicando quatro filtros refinados de criticidade: alta entropia de política (Shannon entropy / top-2 ratio $\ge 0,5$), incerteza de valor e corrida ($|V| \approx 0$, $|d_{\text{own}} - d_{\text{opp}}| \le 2$), mudanças críticas de muro ($\le 3$ muros ou labirinto denso com alta pressão CAT) e swings de virada. **Supervisão com busca real em árvore ("search search mesmo")**: **80% Claustrophobia MCTS** (`zq_search_bridge.exe` 64 sims em CUDA) + **20% ZQuoridor MCAB** (512 nós em CPU), ponderados por divergência Jensen-Shannon. | **$\times 4,5$** | **100.000 buscas Claustrophobia MCTS 100% Concluídas na GPU** (`claustro-search.npz`). Busca ZQuoridor MCAB 512 nós em execução ativa na CPU (`task-3396`). 15.000 amostras de validação isoladas. |
