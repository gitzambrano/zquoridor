...
int main(int argc, char** argv) {
    ...
    while (std::getline(std::cin, line)) {
        ...
        if (root == nullptr || root->moves.empty()) {
            if (state.wallsLeft[0] == 0 && state.wallsLeft[1] == 0) {
                // The production root solver selects a move without an MCTS tree.
                const int action = canonicalPolicyIndex(best, state.turn);
                const double value = stats.score > qr::SCORE_INF / 2 ? 1.0
                    : stats.score < -qr::SCORE_INF / 2 ? 0.0 : 0.5;
                std::cout << "{\"id\":\"" << id << "\",\"side_to_move\":" << state.turn
                          << ",\"best_action\":" << action << ",\"root_value_prob\":" << value
                          << ",\"source\":\"empty_handed_solver\",\"edges\":[["
                          << action << ",1," << value << "]]}\n" << std::flush;
                continue;
            }
